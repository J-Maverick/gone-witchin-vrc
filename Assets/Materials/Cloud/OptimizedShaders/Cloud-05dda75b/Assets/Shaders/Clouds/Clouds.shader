...
Shader "Hidden/Moriohs Shaders/Enviroment Shaders/Clouds/Cloud-05dda75b"
{
	Properties
	{
...
#define PROP_CULLMODE 1
#define PROP_CULLINGDISTANCE 5000
#define PROP_CULLINGFALLOFF 4
#define PROP_TRANSITIONDISTANCE 50
#define PROP_TRANSITIONFALLOFF 2
#define PROP_EDGESMOOTHINGMIN 0.272
#define PROP_EDGESMOOTHINGMAX 2.36
#define PROP_DIRECTIONALINTENSITY 2
#define PROP_AMBIENTINTENSITY 2
			...
			fixed4 frag (v2f i , FRONT_FACE_TYPE ase_vface : FRONT_FACE_SEMANTIC) : SV_Target
			{
				...
				float4 appendResult93 = (float4(( ( ase_lightColor.rgb * saturate( ( 1.0 - dotResult96 ) ) * float(2) ) + ( localAmbient85 * 2.0 * float(2) ) ) , 1.0));
				float4 MainColor251 = ( appendResult93 * float4(1,1,1,1) );
				...
				float smoothstepResult265 = smoothstep( float(0.272) , float(2.36) , switchResult76);
				...
				float distanceDepth31 = abs( ( screenDepth31 - LinearEyeDepth( ase_screenPosNorm.z ) ) / ( float(50) ) );
				float4 appendResult39 = (float4(break253.x , break253.y , break253.z , ( break253.w * ( ( saturate( pow( ( temp_output_23_0 / float(50) ) , float(2) ) ) * smoothstepResult265 * saturate( distanceDepth31 ) ) * saturate( pow( ( temp_output_23_0 / float(5000) ) , ( 1.0 - float(4) ) ) ) ) )));
